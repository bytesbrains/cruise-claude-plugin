...
*(Note: `/cruise:budget` and `/cruise:status` provide the same real-time budget and status information.)*
...
| Missing / Unset Key | `CRUISE_API_KEY` is not present in the environment. | Set `CRUISE_API_KEY` in your shell (e.g. `~/.zshrc`), restart Claude Code, and run `/cruise:budget` again. |
